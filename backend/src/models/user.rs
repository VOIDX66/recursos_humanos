...
fn validate_role(rol: &str) -> Result<(), ValidationError> {
    ...
    if allowed_roles.contains(&rol) {
        ...
        error.message =
            Some("Role must be one of: admin, analista, postulante, evaluador, instructor".into());
...
pub struct NewUser {
    #[validate(length(
        min = 6,
        max = 12,
        message = "ID number must be between 6 and 12 characters"
    ))]
    pub id_number: String,
...
pub struct Claims {
    ...
    pub user_id: String, // ID del usuario (para identificarlo luego)
    pub role: String,    // Rol del usuario (analista, evaluador, etc.)
