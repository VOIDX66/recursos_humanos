...
pub async fn create_user_handler(
    ...
    if claims.is_none() {
        if new_user.rol != "aplicante" {
            return Err(AppError::Unauthorized("Only applicants can register without being logged in".into()));
        }
    } else {
        ...
        if claims.rol != "admin" {
            return Err(AppError::Unauthorized("Only admins can create users with roles other than applicant".into()));
        }
    }
    ...
    match user_services::create_user(&conn, new_user.into_inner(), claims.as_ref().map(|c| c.rol.clone())).await {
...
pub async fn register_user_handler(
    ...
    if let Err(e) = new_user.validate() {
        return Err(AppError::ValidationError(format!("Validation failed: {:?}", e)));
    }
...
pub async fn login_user_handler(
    ...
    let token = user_services::login_user(&conn, login_data.into_inner(), &app_state.jwt_secret).await?;
...
pub async fn profile_handler(
    ...
    let claims = req.extensions().get::<Claims>().cloned().ok_or_else(|| {
        AppError::AuthenticationError("User claims not found".to_string())
    })?;
